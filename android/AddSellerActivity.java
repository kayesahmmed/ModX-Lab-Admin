...
public class AddSellerActivity extends AppCompatActivity {
	...
	private void initialize(Bundle _savedInstanceState) {
				...
				if (user.getText().toString().equals("")) {
					...
					if (token.getText().toString().equals("")) {
						...
						if (dollar.getText().toString().equals("")) {
							((EditText)dollar).setError("Set Amount!");
						} else {
							if (Device.equals("")) {
								...
								// SECURITY: key generation + database write happen in the "addSeller" Cloud Function.
								AdminApi.addSeller(user.getText().toString(), token.getText().toString(), Device, dollar.getText().toString(), new AdminApi.Callback() {
									@Override
		...
		linear6.setOnClickListener(new View.OnClickListener() {
			@Override
			public void onClick(View _view) {
				{PopupMenu popup = new PopupMenu(AddSellerActivity.this, linear6);
	...
	private void initializeLogic() {
		...
		((LinearLayout)l3).removeView(dollar); 
...
*/
	...
	public void _ProgresbarShow(final String _title) {
		prog = new ProgressDialog(AddSellerActivity.this);
		prog.setMax(100);
